function plot_mode5_data(mat_file)
%PLOT_MODE5_DATA Plot mode 5 tracking, PSD, and MSE figures.

if nargin < 1 || isempty(mat_file)
    mat_file = find_latest_result('mode5');
end
...
fig1 = figure('Name', 'Mode 5 UAV Position Tracking', 'Color', 'w');
for i = 1:3
    subplot(3,1,i);
    ...
    grid on;
    if i == 1
        title('Mode 5 UAV Three-Axis Position Tracking');
...
fig2 = figure('Name', 'Mode 5 Arm Tracking', 'Color', 'w');
for i = 1:3
    subplot(3,1,i);
    ...
    grid on;
    if i == 1
        title('Mode 5 Arm Three-Axis Tracking');
        legend('Desired', 'Actual', 'Location', 'best');
    end
end
xlabel('Time (s)');
...
fig3 = figure('Name', 'Mode 5 Measurement PSD', 'Color', 'w');
...
fig4 = figure('Name', 'Mode 5 UAV Running MSE', 'Color', 'w');
