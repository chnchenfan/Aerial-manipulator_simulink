...
if nargin < 1 || isempty(mode3_file)
    mode3_file = fullfile(pwd, 'tuning_results', 'final_default_validation', ...
        'final_default_mode3_20260427_141819.mat');
end
if nargin < 2 || isempty(mode5_file)
    mode5_file = fullfile(pwd, 'tuning_results', 'final_default_validation', ...
        'final_default_mode5_20260427_141944.mat');
end
...
title([title_prefix ' - 3D Trajectory']);
legend('Desired', 'Actual', 'Location', 'best');
...
for i = 1:3
    ...
    if any(isfinite(data.h_v_true(:, i)))
        plot(data.t, data.h_v_true(:, i), 'r--', 'LineWidth', 1.0);
        legend('ESO estimate', 'Model disturbance', 'Location', 'best');
    else
        legend('ESO estimate', 'Location', 'best');
    end
    grid on;
    ylabel(sprintf('h_v %s', labels{i}));
    if i == 1
        title([title_prefix ' - Position ESO Disturbance Estimate']);
    end
end
xlabel('Time (s)');
