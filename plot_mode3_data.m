function plot_mode3_data(mat_file)
%PLOT_MODE3_DATA Plot mode 3 tracking, PSD, and MSE figures.

if nargin < 1 || isempty(mat_file)
    mat_file = find_latest_result('mode3');
end
...
fig1 = figure('Name', 'Mode 3 UAV Position Tracking', 'Color', 'w');
for i = 1:3
    subplot(3,1,i);
    ...
    grid on;
    if i == 1
        title('Mode 3 UAV Three-Axis Position Tracking');
...
fig2 = figure('Name', 'Mode 3 Arm Tracking', 'Color', 'w');
for i = 1:3
    subplot(3,1,i);
    ...
    grid on;
    if i == 1
        title('Mode 3 Arm Three-Axis Tracking');
        legend('Desired', 'Actual', 'Location', 'best');
    end
end
xlabel('Time (s)');
...
fig3 = figure('Name', 'Mode 3 Measurement PSD', 'Color', 'w');
...
fig4 = figure('Name', 'Mode 3 UAV Running MSE', 'Color', 'w');
